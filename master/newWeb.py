...
class WebUI:
    ...
    def __init__(self, environment, host, port, masterHost, auth_credentials=None, tls_cert=None, tls_key=None,
        ...
        def swarm():
            ...
            client_list = request.url.split('?')[1].split('&')
            # run_seconds=None
            ...
            # print("--------------host======="+str(environment.host))
            ...
            #     environment.runner.stop(client_id)
            ...
            # for id in client_id:
            #     environment.runner.start(user_count, hatch_rate, id)
            environment.runner.start(user_count,hatch_rate,client_list)
            return jsonify({'success': True, 'message': 'Swarming started', 'host': environment.host})

        @app.route('/stop', methods=["POST"])
        @self.auth_required_if_enabled
        def stop():
            client_id = request.url.split('?')[1].split('&')
            interface_id= int(request.form["interface_id"])
            self.reporter_running_status = False  # 结束指标历史记录
            ...
            #
            # else:
            res = environment.runner.stop(client_id,interface_id)
            return jsonify(res)
        ...
        def makeHttpItem(request, prefixStr, i):
            ...
            :return:
            """
            print("打印请求值" + str(request.values))
            xPretask = {}
        ...
        def saveTransation():
            ...
            print("--------打印ID-----" + TestTaskIdMark + TestTaskMark)
            ...
            # print(json.dumps(transObj,indent=4))
            try:
                with open("./jsons/main.json", mode="w+") as f:
        ...
        def request_stats():
            ...
            if is_distributed:
                ...
                report["slaves"] = [{
                    "slave": x,
                    "clientId": (lambda t: "-" if not t or not t in [w.get("id") for w in workers] else t)(
                        self.workedServser.get(x)),
