...
PERCENTILES_TO_DREPORT = [0.90, 0.95]
#less indicators for django
...
class StatsEntry(object):
    ...
    def percentile_to_django(self):
        if not self.num_requests:
            raise ValueError("Can't calculate percentile on url with no successful requests")



        return  [self.get_response_time_percentile(p) for p in PERCENTILES_TO_DREPORT]
...
def setup_distributed_stats_event_listeners(events, stats, clients):
    ...
    def on_worker_report(client_id, data):
        ...
        for stats_data in data["stats"]:
            ...
            #淦，这里最后生成的是一个key为request_key,value为对象的字典

            if not request_key in clients[client_id].stats.entries:
                clients[client_id].stats.entries[request_key] = StatsEntry(stats, entry.name, entry.method,
                                                                           use_response_times_cache=True)
            clients[client_id].stats.entries[request_key].extend(entry)
        ...
        for error_key, error in data["errors"].items():
            ...
            if error_key not in clients[client_id].stats.errors:
                clients[client_id].stats.errors[error_key] = StatsError.from_dict(error)
            else:
                clients[client_id].stats.errors[error_key].occurrences += error["occurrences"]

        stats.total.extend(StatsEntry.unserialize(data["stats_total"]))
        clients[client_id].stats.total.extend(StatsEntry.unserialize(data["stats_total"]))
